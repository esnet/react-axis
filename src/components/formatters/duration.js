...
export default function(duration, template, precision) {
    ...
    // types
    types = settings.types = (_.isArray(settings.types) ?
        settings.types : settings.types.split(" "));
    ...
    tokens = _.map(tokens, (token) => {
        ...
        if (token.isLeast && (settings.precision > 0)) {
            ...
            switch (decVal.length) {
                case 1:
                    val += "." + padZero(decVal[0], settings.precision, true)
                        .slice(0, settings.precision);
                    break;
                    
                case 2:
                    val += "." + padZero(decVal[1], settings.precision, true)
                        .slice(0, settings.precision);
                    break;
                    
                case 3:
                    val += "." +
                        padZero(repeatZero((+decVal[2]) - 1) +
                                (decVal[0] || "0") +
                                decVal[1], settings.precision, true)
                        .slice(0, settings.precision);
                    break;
